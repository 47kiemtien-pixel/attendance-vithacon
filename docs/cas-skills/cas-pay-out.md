---
name: cas-pay-out
description: "Tích hợp Pay Out của Cas (scopes: transfer): các bước tích hợp, API cần gọi và checklist trước khi lên production."
---

# Cas — Pay Out

> Nguồn: https://cas.so/product/pay-out · Markdown: https://cas.so/product/pay-out.md
...
Khi tích hợp, mở rộng hoặc gỡ lỗi **Pay Out** của Cas trong hệ thống của bạn.
...
1. Tạo một phân quyền [/grant/token](https://cas.so/general/api/grant/create.md) với `scopes` có giá trị là `transfer`.
...
4. Bây giờ bạn đã có thể gọi [API chuyển tiển](https://cas.so/general/api/transfer.md).

## API dùng trong luồng này

- [Tạo đối tượng phân quyền](https://cas.so/general/api/grant/create.md)
- [Transfer](https://cas.so/general/api/transfer.md)
